...
% intersects(+Range, +Range) --> returns whether two ranges are intersected
...
% leftMinDistantNode(+Range, +Range, +Node, +Node) --> returns whether left node is closer to certain range wrt right node
...
% Element to remove isn't in tree
remove(_, T,T).
